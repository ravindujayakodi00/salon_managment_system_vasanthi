...
export default function AvailableStylistsView({
    ...
    const realStylists = stylistsWithSlots.filter(({ stylist }) => stylist.id !== 'NO_PREFERENCE');
    // If the backend couldn't return detailed stylists, keep the old "Any Professional" fallback.
    const dropdownStylists = realStylists.length > 0 ? realStylists : stylistsWithSlots;
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-2">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    ...
                    <span className="px-3 py-1 text-sm font-medium bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 rounded-full sm:self-start">
                        {dropdownStylists.length} options
                    </span>
                </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                    ...
                    <select
                        ...
                        onChange={(e) => {
                            ...
                            setSelectedStylist(next);
                            setSelectedTime(null); // reset selected time when stylist changes
                        }}
                        className="w-full sm:w-72 px-3 py-2.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-sm text-gray-900 dark:text-white"
                    >
                        <option value="">
                            Select stylist
                        </option>
                        {dropdownStylists.map(({ stylist }) => (
                            <option key={stylist.id} value={stylist.id}>
                                {stylist.id === 'NO_PREFERENCE' ? 'Any Professional (Fastest)' : stylist.name}
                            </option>
                        ))}
                    </select>
                </div>
            ...
            {!selectedStylist ? (
                <div className="text-sm text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
                    Select a stylist to see available time slots.
                </div>
            ) : (
                <div className="space-y-4 max-h-[65dvh] overflow-y-auto pr-0 sm:pr-2">
                    <AnimatePresence mode="wait">
                        {(() => {
                            const selectedEntry = stylistsWithSlots.find(s => s.stylist.id === selectedStylist);
                            if (!selectedEntry) return null;

                            const { stylist, slots, skillDetails } = selectedEntry;
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="font-semibold text-green-800 dark:text-green-200">
                                {stylistsWithSlots.find(s => s.stylist.id === selectedStylist)?.stylist.id === 'NO_PREFERENCE'
                                    ? 'Any Professional'
